...
import { restorationParentReference } from '@shared/fidelityDecision'
...
import { hashFile } from './verifiedParent'
import {
  applyModelRepairs,
  ensureModelsDir,
  getModelStatus,
...
function registerIpc(): void {
  ...
  ipcMain.handle(CHANNEL.exportImage, async (_event, options: ExportOptions, params: EditParams): Promise<ExportResult> => {
    ...
    try {
      ...
      const configuration = migrated.fidelity?.configurationId
        ? (await fidelityConfigurations().list()).find((item) => item.id === migrated.fidelity?.configurationId) ?? null
        : null
      const prepared = await prepareFidelityExport(session.sourcePath, session.decode, params, migrated, configuration)
    ...
    async (
      ...
      try {
        ...
        const configuration = migrated.fidelity?.configurationId
          ? (await fidelityConfigurations().list()).find((item) => item.id === migrated.fidelity?.configurationId) ?? null
          : null
        const prepared = await prepareFidelityExport(sourcePath, decode, params, migrated, configuration)
