import { Ban, Pipette, RefreshCw, Sparkles } from 'lucide-react'
  ...
  return (
    <>
      ...
      <div className="section">
        ...
        <p className="hint">
          齿孔、漏光边这类区域本身接近纯黑，参与统计会把片基 / 对齐的参考值整体拉低。
          进入框选后在画面上拖拽即可标记，单击已标记区域可将其删除；
          标记只从去色罩统计中剔除，画面本身不受影响。
        </p>
      </div>

      <div className="section">
        <div className="section-head">
          <span className="section-title">校正方式</span>
        </div>
        <div className="chips">
          <button className="chip is-active" disabled>
            通道对齐
          </button>
        </div>
        <p className="hint">
          统一使用通道对齐：先把三通道黑场/白场对齐消除色罩偏色，再做密度反相。
          片基反相模式已停用（多数翻拍样片上效果更差）。
        </p>
      </div>
